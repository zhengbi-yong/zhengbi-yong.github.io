...
export default function HeroCard({
  ...
  const webCaseRef = useRef<HTMLDivElement>(null)
  ...
  const caseGoRef = useRef<HTMLDivElement>(null)
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      mouseXValue = e.clientX / window.innerWidth - 0.5
      mouseYValue = e.clientY / window.innerHeight - 0.5
    }

    document.addEventListener('mousemove', handleMouseMove)

    // 视差动画循环
    let parallaxAnimationId: number
    const updateParallax = () => {
      if (webCaseRef.current) {
        ...
        const newY = currentY + (targetY - currentY) * parallaxEasing
        webCaseRef.current.style.transform = `translate3d(${newX}px, ${newY}px, 0)`
      }
      ...
      if (colorPickerRef.current) {
        ...
        const newX = currentX + (targetX - currentX) * parallaxEasing
        const newY = currentY + (targetY - currentY) * parallaxEasing
        colorPickerRef.current.style.transform = `translate3d(${newX}px, ${newY}px, 0)`
      }
      ...
      if (colorRef.current) {
        ...
        const newX = currentX + (targetX - currentX) * parallaxEasing
        const newY = currentY + (targetY - currentY) * parallaxEasing
        colorRef.current.style.transform = `translate3d(${newX}px, ${newY}px, 0)`
      }
    ...
    const updateCursor = () => {
      if (isHovering && cursorIndicatorRef.current) {
        const { x: targetX, y: targetY } = targetPositionRef.current
        cursorXValue += (targetX - cursorXValue) * cursorEasing
        cursorYValue += (targetY - cursorYValue) * cursorEasing
        cursorIndicatorRef.current.style.left = `${cursorXValue}px`
        cursorIndicatorRef.current.style.top = `${cursorYValue}px`
      }
  ...
  // 处理鼠标在图片区域移动
  const handleCaseGoMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    if (!isHovering || !isMounted || typeof window === 'undefined' || window.innerWidth < 768)
      return
    const rect = caseGoRef.current?.getBoundingClientRect()
    if (rect) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <path
            d="M173.709 22.3152L177.728 26.3835L187.375 16.6164"
            stroke="#4A3AFF"
            strokeWidth="2.32184"
            strokeLinecap="round"
      ...
      >
        ...
        />
        <defs>
          <linearGradient id="paint0_linear" x1="62.8359" y1="12.4294" x2="263.408" y2="12.4294">
            <stop stopColor="#FF0000" />
            <stop offset="0.145833" stopColor="#FF6B00" />
