...
1. 当访问不存在的路径（如 `/pre/slidev1/0`）时
2. GitHub Pages 查找该路径的文件，找不到
3. GitHub Pages 返回同目录下的 `404.html` 内容
4. **重要**：URL 保持不变（仍然是 `/pre/slidev1/0`）
5. Slidev 的路由系统读取 `window.location.pathname`
6. 根据路径显示对应的幻灯片（第 0 页）
