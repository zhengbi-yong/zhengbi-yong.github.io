...
> **注意**: 亮/暗模式由 `next-themes` 管理 (ThemeProvider + useTheme)。**颜色主题**由 Zustand `theme-store` 管理，两者独立并存。
...
```text
Layer 0: geist-tokens.css          → 原始值（Vercel Geist），亮/暗两套
Layer 1: tokens.css                → 统一语义层（--theme-accent, --theme-bg, …）
Layer 2: themes/*.css (32个)       → 主题色板，通过 [data-theme] 切换
Layer 3: tailwind.css              → @theme 映射 + 全局样式
Layer 4: admin-theme.css           → Admin 专用（--admin-*）
Layer 5: visitor-theme.css         → 访客主题（--visitor-*）
Layer 6: monograph-theme.css       → 长文阅读主题（--monograph-*）
```

> **主题系统**：32 套配色通过 `<html data-theme="xxx">` + CSS 变量切换。详见 [features/theme-system.md](../features/theme-system.md)。
> **颜色主题**由 Zustand `theme-store` 管理，与 `next-themes` 的亮/暗模式独立并存。
