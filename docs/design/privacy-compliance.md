# 数据隐私与合规（规划）

> ⚠️ **重要说明：以下所有内容均为规划阶段，尚未实施。当前项目尚未部署任何隐私合规功能（GDPR / 个人信息保护法 / CCPA）。**

> 来源：EDITOR_SYSTEM_DESIGN.md P8 — 扩展阶段，尚未实施

## 合规要求

> ⚠️ **尚未实施** — 以下合规框架仅作为规划参考，目前代码库中无对应实现。

| 法规 | 适用范围 | 核心要求 |
|------|---------|----------|
| GDPR | EU 用户 | 数据可删除、可导出、明确同意 |
| 个人信息保护法 | 中国用户 | 最小必要、告知同意、跨境传输 |
| CCPA | 加州用户 | 知情权、删除权、不出售权 |

## 数据分类

> ⚠️ **尚未实施** — 数据分类与处理策略尚未在代码中实现。

| 类别 | 示例 | 处理方式 |
|------|------|---------|
| 个人身份信息 | 邮箱、IP 地址 | 加密存储、可清除 |
| 认证数据 | 密码哈希、Token | Argon2id 哈希、不可逆 |
| 内容数据 | 文章、评论 | 保留直到用户删除 |
| 行为数据 | 浏览历史、搜索记录 | 匿名化、保留有限期 |

## 自动清除策略 (DROP)

> ⚠️ **尚未实施** — 自动数据清除策略（45 天轮转）尚未在代码中实现。

```
数据保留周期: 45 天
    │
    ├── 日志: 45天后自动轮转
    ├── 搜索历史: 45天后清除
    ├── 未验证用户: 45天后软删除
    └── 已删除文章: 45天后物理删除
```

## 用户数据导出 API

> ⚠️ **尚未实施** — 以下 API 端点尚未实现，无对应路由或 handler。

```
GET /api/v1/user/data-export
Authorization: Bearer <token>

→ 返回 ZIP，包含:
  ├── profile.json     (用户资料)
  ├── articles.json    (所有文章)
  ├── comments.json    (所有评论)
  └── activity.json    (搜索/浏览历史)
```

## 用户数据删除 API

> ⚠️ **尚未实施** — 以下 API 端点尚未实现，无对应路由或 handler。

```
DELETE /api/v1/user/data
Authorization: Bearer <token>
X-XSRF-TOKEN: <csrf_token>

→ 触发异步删除任务
→ 返回 task_id
→ 45天内完成所有数据清除
```
