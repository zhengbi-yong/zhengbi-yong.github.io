...
### `media_assets` 表（独立媒体资源表）

`media_assets` 表（定义在迁移 `2026042701_create_articles.sql` 中）是一个独立媒体资源表，**不是** junction 关联表。它存储媒体文件的元数据并关联到文章：

```sql
CREATE TABLE media_assets (
    id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    filename     VARCHAR(255) NOT NULL,         -- 存储文件名（UUID 格式）
    ...
    article_id   UUID REFERENCES articles(id),  -- 关联文章（可空）
    created_at   TIMESTAMPTZ DEFAULT NOW()
);
```

> **注意**：`media_assets` 是独立的媒体资源表，每行代表一个媒体文件，而非多对多关联表。有别于主 `media` 表，`media_assets` 使用 `VARCHAR` 字段、无软删除列、有 `duration` 和 `article_id` 直接列。
