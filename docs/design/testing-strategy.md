...
```text
       ...
       ╱──────────╲
      ╱ 集成测试   ╲            前端 Vitest (~200 tests), 后端 cargo test
     ╱──────────────╲
    ╱   单元测试      ╲          后端: 11 个集成/压力测试文件 + 4 个核心测试模块
   ╱────────────────────╲
...
| 层级 | 工具 | 目标 | 现状 |
|------|------|------|------|
| Rust 单元测试 | `cargo test` | 核心逻辑 ≥80% | 11 个集成/压力测试文件 + 4 个核心测试模块 |
...
| E2E 流程 | Playwright | 12 条核心路径 | ~100 个 E2E 测试用例 |
