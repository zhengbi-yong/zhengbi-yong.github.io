  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <header className="mb-8">
        ...
        <p className="text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">查看访问网站的用户地理位置分布</p>
      </header>

      {/* 统计信息 */}
      <div className="mb-8 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg)] p-6 shadow-sm dark:border-gray-700">
          <h3 className="mb-1 text-sm font-medium text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">总访客数</h3>
          <p className="text-3xl font-bold text-[var(--theme-fg)]">{totalVisitors}</p>
        </div>
        <div className="rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg)] p-6 shadow-sm dark:border-gray-700">
          <h3 className="mb-1 text-sm font-medium text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">国家/地区数</h3>
          <p className="text-3xl font-bold text-[var(--theme-fg)]">{uniqueCountries}</p>
        </div>
        <div className="rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg)] p-6 shadow-sm dark:border-gray-700">
          <h3 className="mb-1 text-sm font-medium text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">总访问次数</h3>
          <p className="text-3xl font-bold text-[var(--theme-fg)]">{totalVisits}</p>
        </div>
      </div>
      ...
      {/* 访客列表（可选） */}
      {visitors.length > 0 && (
        <div className="mt-8 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg)] p-6 shadow-sm dark:border-gray-700">
          <h2 className="mb-4 text-2xl font-bold">访客列表</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--theme-border)] dark:border-gray-700">
              ...
              <tbody>
                {visitors.map((visitor, index) => (
                  <tr
                    ...
                    className="border-b border-gray-100 hover:bg-[var(--theme-bg-secondary)] dark:border-gray-800 dark:hover:bg-gray-700"
                  >
                    <td className="px-4 py-2 font-mono text-xs">{visitor.ip}</td>
                    <td className="px-4 py-2">
                      {visitor.city}, {visitor.country}
                    </td>
                    <td className="px-4 py-2 text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">
