      ...
      <PublicPageFrame>
        ...
        <div className="mb-12 mt-4">
          <div className="flex items-end justify-between gap-8">
            <div>
              ...
              <p className="mt-3 text-sm text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">
          ...
          {musicSheets.map((music) => (
            <article key={music.id} className="group">
              <Link href={`/music/${music.id}`} className="block">
                <div className="relative overflow-hidden border border-[var(--theme-border)]/70 bg-[var(--theme-bg-secondary)]/90 shadow-[0_30px_80px_-60px_rgba(15,23,42,0.35)] transition-all duration-500 group-hover:-translate-y-1 group-hover:border-amber-700/30 group-hover:bg-[var(--theme-bg)] group-hover:shadow-[0_30px_90px_-58px_rgba(180,83,9,0.28)] dark:border-zinc-800/80 /60 dark:shadow-[0_30px_80px_-60px_rgba(5,8,15,0.95)] dark:group-hover:border-amber-500/20 dark:group-hover:bg-zinc-900/90 dark:group-hover:shadow-[0_30px_90px_-58px_rgba(245,158,11,0.14)]">
                  <div className="relative aspect-[3/4] overflow-hidden bg-gradient-to-b from-zinc-200 to-zinc-100 dark:from-zinc-800 dark:to-zinc-900">
                    ...
                    {music.difficulty && (
                      <div className="absolute right-4 top-4">
                        <span className="inline-block border border-zinc-300/60 bg-[var(--theme-bg)]/80 px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-[var(--theme-fg-secondary)] dark:border-zinc-600/40 /80 dark:text-[var(--theme-fg-tertiary)]">
                          {music.difficulty}
                        </span>
                      </div>
                    )}
                    ...
                    {music.instrument && (
                      <div className="absolute left-4 bottom-4">
                        <span className="inline-block bg-black/40 backdrop-blur-sm px-2.5 py-1 text-[10px] font-medium uppercase tracking-wider text-white/90 dark:bg-[var(--theme-bg)]/10 dark:text-white/80">
                          {music.instrument}
                        </span>
                      </div>
                    )}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="flex items-center justify-center w-14 h-14 rounded-full bg-[var(--theme-bg)]/90 /90 shadow-lg backdrop-blur-sm">
                  ...
                  <div className="px-5 pb-6 pt-4">
                    ...
                    {(music.composer || music.year) && (
                      <div className="mb-3 flex items-center gap-2">
                        {music.composer && (
                          <span className="text-xs font-medium text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">
                            {music.composer}
                          </span>
                        )}
                        {music.composer && music.year && (
                          <span className="text-zinc-300 dark:text-[var(--theme-fg-secondary)]">|</span>
                        )}
                        {music.year && (
                          <span className="text-xs text-[var(--theme-fg-tertiary)] dark:text-[var(--theme-fg-secondary)]">{music.year}</span>
                        )}
                      </div>
                    )}

                    {music.description && (
                      <p className="mb-4 line-clamp-2 text-sm leading-relaxed text-[var(--theme-fg-secondary)] dark:text-[var(--theme-fg-tertiary)]">
                        {music.description}
                      </p>
                    )}
                    ...
                    <div className="border-t border-[var(--theme-border)]/60 pt-3 dark:border-zinc-700/40">
        ...
        {musicSheets.length === 0 && (
          <div className="py-32 text-center">
            <p className="text-[var(--theme-fg-tertiary)] dark:text-[var(--theme-fg-secondary)]">暂无乐谱</p>
          </div>
        )}
      </PublicPageFrame>
    </PublicLayout>
