  ...
  return (
    <section className="border-t bg-gradient-to-r from-primary-50 via-white to-accent-50 py-12 dark:border-gray-700 dark:from-gray-900 dark:via-gray-800 dark:to-indigo-950">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl font-bold text-[var(--theme-fg)] dark:text-white sm:text-4xl">
            {title}
          </h2>
          {description && (
            <p className="mt-3 text-lg text-[var(--theme-fg-secondary)] ">{description}</p>
          )}
        </motion.div>
          ...
          {recommendations.map((item, index) => (
            ...
            >
              <Link href={item.slug} className="block">
                <div className="group relative overflow-hidden rounded-2xl border-2 bg-[var(--theme-bg)] shadow-md transition-all duration-300 hover:shadow-xl dark:border-gray-700">
                  {/* 图片 */}
                  <div className="relative aspect-[16/9] overflow-hidden bg-[var(--theme-bg-tertiary)]">
                    <Image
                  ...
                  <div className="p-5">
                    ...
                    {/* 标题 */}
                    <h3 className="mb-2 line-clamp-2 text-lg font-bold text-[var(--theme-fg)] group-hover:text-primary-600 dark:text-white dark:group-hover:text-primary-400">
                      {item.title}
                    </h3>

                    {/* 摘要 */}
                    {item.summary && (
                      <p className="mb-4 line-clamp-2 text-sm text-[var(--theme-fg-secondary)] ">
                        {item.summary}
                      </p>
                    )}
                  ...
                  {/* 类型指示器 */}
                  <div className="absolute top-3 right-3 flex h-8 w-8 items-center justify-center rounded-full bg-[var(--theme-bg)]/90 backdrop-blur-sm/90">
