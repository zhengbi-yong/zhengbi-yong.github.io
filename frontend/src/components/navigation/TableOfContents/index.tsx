    ...
    <div className={styles.tocContainer} ref={tocContentRef}>
      ...
      <div className={styles.tocTitle}>
        <div className="flex items-center gap-2">
          <List size={16} className="text-muted-foreground dark:text-muted-foreground" />
          <span className={cn(styles.tocTitleText, 'dark:text-foreground')}>目录</span>
        </div>
        <span className={cn(styles.tocProgressLabel, 'dark:text-muted-foreground')}>
          {Math.round(progress)}%
        </span>
      </div>
    ...
    <>
      ...
      <button
        ...
        className={cn(
          ...
          'dark:bg-card dark:text-foreground dark:hover:bg-secondary'
        )}
      >
        <svg
      ...
      <div className={cn(styles.tocMobilePanel, isMobileExpanded && styles.tocMobilePanelOpen)}>
        <div className={styles.tocMobilePanelHeader}>
          <span className={cn(styles.tocMobilePanelTitle, 'font-brand dark:text-foreground')}>
            目录
          </span>
          <button
            onClick={() => setIsMobileExpanded(false)}
            aria-label="Close table of contents"
            className={cn(styles.tocCloseButton, 'dark:text-muted-foreground dark:hover:text-gray-200')}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
