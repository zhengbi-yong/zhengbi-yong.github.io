    ...
    <div className={cn(
      ...
      <div className="overflow-x-auto admin-scrollbar">
        <table className="w-full border-collapse">
          {/* 表头 */}
          <thead>
            <tr className="data-table-header border-b border-border dark:border-border">
              {/* 复选框列 */}
              ...
              {columns.map((column) => (
                <th
                  ...
                  className={cn(
                    ...
                    'whitespace-nowrap cursor-pointer hover:bg-muted dark:hover:bg-secondary/50',
                    'transition-colors select-none',
                    column.sortable && 'hover:text-foreground dark:hover:text-gray-300',
                    column.className
                  )}
            ...
            {sortedData.map((row, index) => {
              ...
              return (
                <tr
                  key={key}
                  className={cn(
                    'data-table-row border-b border-border dark:border-border last:border-b-0',
                    'transition-colors duration-150',
                    onRowClick && 'cursor-pointer hover:bg-muted dark:hover:bg-secondary/50',
                    isSelected && 'bg-[var(--theme-info-muted)] dark:bg-blue-900/10'
                  )}
      ...
      {/* 批量操作栏 */}
      {selectedRows.size > 0 && (
        <div className="sticky bottom-0 z-10 bg-[var(--theme-info-muted)] dark:bg-[var(--theme-info-muted)] border-t border-[var(--theme-info)]/20 dark:border-blue-800 px-4 py-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 text-admin-sm font-medium text-primary dark:text-[var(--theme-fg)] hover:bg-[var(--theme-info-muted)] dark:hover:bg-blue-800/50 rounded transition-colors"
              >
                取消选择
              </button>
              {/* 可以在这里添加批量操作按钮 */}
...
export function DataTableRow<T extends Record<string, any>>({
  ...
  return (
    <tr
      className={cn(
        'data-table-row border-b border-border dark:border-border',
        'transition-colors duration-150',
        onClick && 'cursor-pointer hover:bg-muted dark:hover:bg-secondary/50',
        isSelected && 'bg-[var(--theme-info-muted)] dark:bg-blue-900/10'
      )}
      onClick={onClick}
    >
