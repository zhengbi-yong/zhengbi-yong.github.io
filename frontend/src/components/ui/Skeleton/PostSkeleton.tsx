      ...
      <div className="xl:divide-y xl:divide-gray-200 xl:dark:divide-gray-700">
        ...
        <header className="pt-6 xl:pb-8">
          <div className="space-y-4 text-center">
            {/* 日期骨架 */}
            <div className="mb-6 h-6 w-64 animate-pulse rounded bg-gray-200 dark:bg-secondary mx-auto" />

            {/* 标题骨架 */}
            <div className="space-y-3">
              <div className="h-10 w-3/4 animate-pulse rounded-lg bg-gray-200 dark:bg-secondary mx-auto sm:h-12 md:h-14" />
            </div>

            {/* 标签骨架 */}
            <div className="flex flex-wrap justify-center gap-2">
              {[...Array(3)].map((_, i) => (
                <div key={i} className="h-6 w-20 animate-pulse rounded-full bg-gray-200 dark:bg-secondary" />
              ))}
            </div>
          </div>
        </header>
            ...
            <div className="flex flex-col h-full w-full">
              ...
              <div className="space-y-2">
                {[...Array(6)].map((_, i) => (
                  <div
                    key={i}
                    className="h-4 animate-pulse rounded bg-gray-200 dark:bg-secondary"
                    style={{ width: `${80 - i * 10}%` }}
                  />
                ))}
              </div>
...
const PostContentSkeleton = memo(function PostContentSkeleton() {
  return (
    <div className="space-y-6">
      ...
      {/* 二级标题骨架 */}
      <div className="h-6 w-2/3 animate-pulse rounded bg-gray-200 dark:bg-secondary" />
      ...
      {/* 代码块骨架 */}
      <div className="h-48 w-full animate-pulse rounded-lg bg-gray-200 dark:bg-secondary" />

      {/* 三级标题骨架 */}
      <div className="h-5 w-1/2 animate-pulse rounded bg-gray-200 dark:bg-secondary" />
      ...
      <div className="space-y-2">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="flex items-center gap-2">
            <div className="h-4 w-4 flex-shrink-0 animate-pulse rounded bg-gray-200 dark:bg-secondary" />
            <div className="h-4 flex-1 animate-pulse rounded bg-gray-200 dark:bg-secondary" />
          </div>
        ))}
      </div>
