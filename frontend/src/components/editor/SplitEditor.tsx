...
export function SplitEditor({
  ...
  const renderMarkdown = useCallback((input: string) => {
    if (!input.trim()) {
      return '<p class="text-muted-foreground italic">预览区域</p>'
    }
  ...
  return (
    <div className={cn('flex h-full flex-col bg-background dark:bg-background', className)}>
      {/* Main content area */}
      <div className="flex flex-1 overflow-hidden">
        ...
        {viewMode !== 'preview' && (
          <div
            className={cn(
              'flex border-r border-border dark:border-border',
              viewMode === 'edit' ? 'w-full' : 'w-1/2'
            )}
          >
            {/* Line numbers */}
            <div className="w-14 flex-shrink-0 overflow-hidden border-r border-border bg-muted select-none dark:border-border dark:bg-card">
              <div className="py-4 pr-4 text-right font-mono text-xs leading-[1.6rem] text-muted-foreground dark:text-muted-foreground">
                {lineNumbers.map((num) => (
            ...
            <div className="relative flex-1">
              <textarea
                ...
                className="absolute inset-0 h-full w-full resize-none border-none bg-background p-6 font-mono text-base leading-[1.6rem] text-foreground placeholder-gray-300 focus:ring-0 focus:outline-none dark:bg-background dark:text-foreground dark:placeholder-gray-600"
                spellCheck={false}
              />
            </div>
          </div>
        ...
        {viewMode !== 'edit' && (
          <div
            ...
            className={cn(
              'overflow-auto bg-background dark:bg-background',
          ...
          >
            <div className="mx-auto max-w-3xl p-8">
              ...
              <div
                className="prose prose-lg dark:prose-invert prose-headings:font-bold prose-headings:tracking-tight prose-h1:text-3xl prose-h1:mb-4 prose-h1:mt-8 prose-h2:text-2xl prose-h2:mb-3 prose-h2:mt-6 prose-h3:text-xl prose-h3:mb-2 prose-h3:mt-4 prose-p:leading-relaxed prose-p:mb-4 prose-strong:font-semibold prose-strong:text-foreground dark:prose-strong:text-gray-100 prose-code:bg-secondary dark:prose-code:bg-gray-800 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-sm prose-code:font-mono prose-pre:bg-gray-900 dark:prose-pre:bg-gray-950 prose-a:text-primary dark:prose-a:text-blue-400 prose-a:no-underline hover:prose-a:underline prose-blockquote:border-l-4 prose-blockquote:border-border dark:prose-blockquote:border-gray-600 prose-blockquote:pl-4 prose-blockquote:italic prose-ul:list-disc prose-ul:ml-6 prose-ol:list-decimal prose-ol:ml-6 prose-li:mb-1 prose-img:rounded-lg prose-img:shadow-md prose-hr:border-border dark:prose-hr:border-gray-700 prose-table:border-collapse prose-th:border prose-th:border-border prose-th:px-3 prose-th:py-2 prose-th:bg-secondary dark:prose-th:bg-gray-700 prose-td:border prose-td:border-border prose-td:px-3 prose-td:py-2 max-w-none"
