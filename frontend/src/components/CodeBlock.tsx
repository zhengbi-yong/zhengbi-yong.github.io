...
export default function CodeBlock({ children, className }: CodeBlockProps) {
  ...
  return (
    <div className="code-block rounded-xl overflow-hidden my-6 shadow-md">
      ...
      <div className="code-block-header flex items-center justify-between bg-zinc-900 dark:bg-zinc-950 px-4 py-2.5">
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-red-500/80" />
          <span className="w-3 h-3 rounded-full bg-yellow-500/80" />
          <span className="w-3 h-3 rounded-full bg-green-500/80" />
          <span className="text-xs text-zinc-400 ml-2 font-mono uppercase tracking-wide">
            {lang}
          </span>
        </div>
        <button
          onClick={handleCopy}
          className="flex items-center gap-1.5 px-2 py-1 rounded text-xs text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 transition-colors duration-150"
