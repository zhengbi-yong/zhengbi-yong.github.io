...
const engagementBadgeClasses: Record<EngagementTier, string> = {
  strong: 'bg-gray-200 text-foreground dark:bg-secondary dark:text-foreground',
  subtle: 'bg-secondary text-muted-foreground dark:bg-card dark:text-muted-foreground',
}
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 text-sm text-muted-foreground dark:text-muted-foreground">
        ...
        {displayIsPopular && <TrendingUp size={14} className="text-muted-foreground dark:text-muted-foreground" />}
      </div>
    )
  }

  return (
    <div className="rounded-lg border border-border bg-muted p-4 dark:border-border dark:bg-card">
      {/* 热度标签 */}
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span
            className="text-sm font-bold text-muted-foreground dark:text-foreground"
            suppressHydrationWarning
          >
            {displayPopularityLabel}
          </span>
          {displayIsPopular && <TrendingUp size={16} className="text-muted-foreground dark:text-muted-foreground" />}
          {displayAnalytics.engagementScore >= 80 && <Award size={16} className="text-muted-foreground dark:text-muted-foreground" />}
        </div>
      </div>
      ...
      <div className="mb-4">
        <div className="mb-1 flex items-center justify-between">
          <span className="text-xs text-muted-foreground dark:text-muted-foreground" suppressHydrationWarning>
        ...
        <div className="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-secondary">
          <div
      ...
      {showDetails && (
        <div className="grid grid-cols-2 gap-4 text-sm">
          {/* 浏览次数 */}
          <div className="flex items-center gap-2">
            <Eye size={16} className="text-muted-foreground dark:text-muted-foreground" />
            <div>
              <p className="text-xs text-muted-foreground dark:text-muted-foreground" suppressHydrationWarning>
                {displayText('analytics.views')}
              </p>
              <p className="font-medium text-foreground dark:text-foreground" suppressHydrationWarning>
                {displayAnalytics.viewCount}
              </p>
            </div>
          </div>
          ...
          {/* 平均阅读时间 */}
          <div className="flex items-center gap-2">
            <Clock size={16} className="text-muted-foreground dark:text-muted-foreground" />
            <div>
              <p className="text-xs text-muted-foreground dark:text-muted-foreground" suppressHydrationWarning>
          ...
          {/* 滚动深度 */}
          <div className="flex items-center gap-2">
            <BarChart3 size={16} className="text-muted-foreground dark:text-muted-foreground" />
            <div>
              <p className="text-xs text-muted-foreground dark:text-muted-foreground" suppressHydrationWarning>
          ...
          {isMounted && analytics.lastVisited && (
            <div className="flex items-center gap-2">
              <TrendingUp size={16} className="text-muted-foreground dark:text-muted-foreground" />
              <div>
                <p className="text-xs text-muted-foreground dark:text-muted-foreground" suppressHydrationWarning>
...
export function PopularArticles({ limit = 5, excludeId }: PopularArticlesProps) {
  ...
  return (
    <div className="flex h-full flex-col rounded-xl border border-border/60 bg-[#F5F3F0] shadow-sm dark:border-border dark:bg-background" style={{ height: '100%', maxHeight: '100%' }}>
      <div className="flex-shrink-0 border-b border-border/40 px-4 py-3 dark:border-border">
        <h3
          className="flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground dark:text-foreground"
          suppressHydrationWarning
        >
          <TrendingUp size={16} className="text-muted-foreground dark:text-muted-foreground" />
        ...
        {popularArticles.map(({ articleId, analytics }, index) => {
          ...
          return (
            <Link
              key={articleId}
              href={path ? `/${path}` : '#'}
              className="group block px-4 py-2.5 transition-colors hover:bg-background/50 dark:hover:bg-gray-800/50"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  {/* 排名和标题 */}
                  <div className="flex items-start gap-2 mb-2">
                    <span className="flex-shrink-0 flex h-5 w-5 items-center justify-center rounded-full text-xs font-semibold text-muted-foreground dark:text-muted-foreground bg-secondary dark:bg-card">
                      {index + 1}
                    </span>
                    <h4 className="line-clamp-2 text-sm font-medium leading-snug text-foreground group-hover:text-primary-600 dark:text-foreground dark:group-hover:text-primary-400 transition-colors">
                      {title}
                    </h4>
                  </div>
                  
                  {/* 统计数据 */}
                  <div className="flex items-center gap-4 text-xs text-muted-foreground dark:text-muted-foreground ml-7">
                    <span className="flex items-center gap-1" suppressHydrationWarning>
                ...
                <div className="flex-shrink-0">
                  <div className="flex flex-col items-end gap-1.5">
                    ...
                    {/* 进度条 */}
                    <div className="w-12 h-1 overflow-hidden rounded-full bg-gray-200 dark:bg-secondary">
                      <div
                        className={cn(
