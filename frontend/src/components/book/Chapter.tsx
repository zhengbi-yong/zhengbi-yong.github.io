  ...
  return (
    <div className="relative mb-6">
      ...
      {chapter.name && (
        <button
          onClick={toggleOpen}
          className="mb-4 ml-6 flex w-full items-center justify-between rounded-xl border border-border/60 bg-gradient-to-r from-white/80 to-gray-50/80 px-6 py-4 text-left shadow-md backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:from-white hover:to-gray-100 hover:shadow-lg dark:border-border/60 dark:from-gray-800/80 dark:to-gray-900/80 dark:hover:from-gray-700 dark:hover:to-gray-800"
            ...
            <div
              className={`h-3 w-3 rounded-full transition-all duration-300 ${
                isOpen
                  ...
                  : 'bg-gray-400 dark:bg-muted0'
              }`}
            ></div>
            <h3 className="text-base font-bold text-foreground dark:text-foreground">{chapter.name}</h3>
          </div>
          <span className="rounded-full bg-secondary/80 px-4 py-1.5 text-sm font-semibold text-muted-foreground dark:bg-card/80 dark:text-foreground">
