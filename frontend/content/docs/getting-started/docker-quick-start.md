...
编辑 `deployments/docker/compose-files/prod/docker-compose.yml` 添加资源限制：
...
修改 `deployments/docker/compose-files/dev/docker-compose.yml`:
...
# 终止进程或修改 `deployments/docker/compose-files/dev/docker-compose.yml` 中的端口映射
```
...
# 增加资源限制（编辑 `deployments/docker/compose-files/prod/docker-compose.yml`）
# 见上文"资源限制"部分
