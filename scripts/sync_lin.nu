...
try {
    # 使用 timeout 命令限制 lint 执行时间（30秒）
    let lint_result = (^timeout 30 pnpm lint | complete)
    if $lint_result.exit_code != 0 {
        # 检查是否是超时
        if ($lint_result.stderr | into string | str contains "timeout") {
    ...
    write-warning-msg $"代码检查过程出错，但继续执行: ($in)"
}
...
let rsync_source = (if not ($source_folder | str ends-with "/") {
    $source_folder + "/"
} else {
    $source_folder
})

print ("源路径: " + $rsync_source)
...
try {
    ...
    # 构建完整的命令参数
    let rsync_target = $"($remote_user)@($remote_ip):($remote_path)"
    
    print $"执行命令: rsync ($rsync_args | str join ' ') -e \"($ssh_command)\" ($rsync_source) ($rsync_target)"
    
    # 执行 rsync 命令
    # 使用 nushell 的 ^ 操作符执行外部命令，展开参数数组
    let rsync_output = (^rsync ...$rsync_args -e $ssh_command $rsync_source $rsync_target | complete)
